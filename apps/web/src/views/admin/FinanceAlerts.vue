  ...
  <div>
    ...
    <el-table v-loading="loading" :data="list" border>
      ...
      <el-table-column label="差异金额" width="120" align="right">
        <template #default="{ row }">
          <span style="color: #ef4444; font-weight: 600">¥{{ (row as Alert).diffAmount }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="120" fixed="right">
        <template #default="{ row }">
          <el-button v-if="!(row as Alert).resolved" link type="success" size="small" @click="onResolve(row as Alert)">
            标记解决
          </el-button>
